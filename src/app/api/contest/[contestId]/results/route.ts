...
export async function GET(request: Request, segmentData: { params: Params }) {
	...
	try {
		const { data } = await cookiesClient.models.Contest.get({ contestId: params.contestId });

		if (!data) {
			throw new Error('Edition not found');
		}

		const editionsResp = (await data.editions()).data;

		if (!editionsResp) {
			throw new Error('Contest has no editions.');
		}

		const promises = editionsResp
			.filter((e) => e.phase !== 'SUBMISSION' && e.phase !== 'VOTING')
			.map(async (edition) => {
				const submissionsResp = (await edition.submissions()).data;
				const contestResp = await edition?.contest();
				const rankingsResp = await edition?.rankings();
		...
		return NextResponse.json({ contestName: data.name, editions: returnData });
