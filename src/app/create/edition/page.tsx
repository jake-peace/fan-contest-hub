...
const CreateEdition: React.FC = () => {
	...
	const [votingSwitch, setVotingSwitch] = useState(false);
	...
	const handleSubmit = async (data: z.infer<typeof FormSchema>) => {
		...
		const { errors, createdEdition } = await createEdition(client, {
			id: newEditionId,
			contestId: contestId as string,
			name: data.name,
			description: data.description as string,
			open: data.submissionsOpen.valueOf(),
			subDeadline: deadlineSwitch ? getUnixTime(data.submissionDeadline) : undefined,
			voteDeadline: votingSwitch ? getUnixTime(data.votingDeadline) : undefined,
		});
								...
								<form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
									...
									<FormField
										...
										render={({ field }) => (
											<FormItem>
												...
												<FormDescription>Submissions will open at this date and time.</FormDescription>
												<FormMessage />
											</FormItem>
										)}
									></FormField>
									...
									<FormField
										...
										render={({ field }) => (
											<FormItem>
												<FormLabel>
													<Switch checked={deadlineSwitch} onCheckedChange={() => setDeadlineSwitch(!deadlineSwitch)} />
													<CalendarIcon className="w-4 h-4" />
													Set a Submission Deadline
												</FormLabel>
												<FormControl>
													<DateTimeHandler disabled={!deadlineSwitch} value={field.value} onChange={field.onChange} />
												</FormControl>
												<FormDescription>{`(Optional) Set a deadline for submitting entries to the edition`}</FormDescription>
												<FormMessage />
											</FormItem>
										)}
									></FormField>
									<FormField
										...
										render={({ field }) => (
											<FormItem>
												<FormLabel>
													<Switch checked={votingSwitch} onCheckedChange={() => setVotingSwitch(!votingSwitch)} />
													<CalendarIcon className="w-4 h-4" />
													Set a Voting Deadline
												</FormLabel>
												<FormControl>
													<DateTimeHandler disabled={!votingSwitch} value={field.value} onChange={field.onChange} />
												</FormControl>
												<FormDescription>{`(Optional) Set a deadline for participants to vote`}</FormDescription>
												<FormMessage />
											</FormItem>
										)}
									></FormField>
