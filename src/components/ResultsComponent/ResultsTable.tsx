...
// 	const aggregateVotes = (votes: JuryPivotTableProps['juryVotes'], songs: SubmissionWithScore[]): AggregatedRow[] => {
...
// 		return Array.from(submissionMap.values()).sort((a, b) => b.totalPoints - a.totalPoints);
// 	};
...
// 				const topVotes = row.votesReceived.filter((v) => v.points >= 8).sort((a, b) => b.points - a.points);
...
// 							{/* Total Score */}
// 							<div className="text-xl font-extrabold px-3 py-1 rounded-full bg-indigo-600 text-white ml-2">{row.totalPoints}</div>
// 						</div>
...
// 								</span>
// 							))}
// 							{topVotes.length === 0 && <span className="text-xs italic text-gray-500">No top points received.</span>}
// 						</div>
// 					</div>
// 				);
// 			})}
...
const JuryPivotTable: React.FC<JuryPivotTableProps> = ({ juryVotes, televotes, submissions }) => {
	const data = generateJuryPivotTable(
		juryVotes,
		televotes,
		submissions.map((s) => s.userId as string),
	...
	return (
		<div id="jury-pivot-table" className="p-2 shadow-xl rounded-lg max-w-full">
			<Card>
				<Table>
					<TableHeader>
						<TableRow>
							{data.headers.map((head) => (
								<TableHead
									key={head}
									className={`px-[0.25vw] py-[0.25vw] lg:px-3 lg:py-3 text-[1.5vw] lg:text-sm font-medium uppercase tracking-wider text-center`}
								>
									{head}
								</TableHead>
							))}
							...
							<TableRow key={row.country}>
								...
								{row.pointsByJury.map((point) =>
									point.points === 0 ? (
										<TableCell
											key={`${point.voteId}`}
											className="whitespace-nowrap text-[1.5vw] lg:text-sm text-center font-semibold"
										></TableCell>
									) : (
										<TableCell key={`${point.voteId}`} className={`whitespace-nowrap text-[1.5vw] text-center font-semibold `}>
											<div className={`rounded-md font-semibold text-[2vw] m:text-xl ${getCellColour(point.points)} m-[0.125vw]`}>
												{point.points}
											</div>
										</TableCell>
									)
								)}
								<TableCell className={`whitespace-nowrap text-[1.5vw] lg:text-sm font-bold text-center`}>{row.televotes}</TableCell>
								<TableCell className={`whitespace-nowrap text-[1.5vw] lg:text-sm font-bold text-center`}>{row.total}</TableCell>
							</TableRow>
						))}
					</TableBody>
				</Table>
