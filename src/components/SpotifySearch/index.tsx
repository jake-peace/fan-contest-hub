...
const SpotifySearch: React.FC<SpotifySearchProps> = ({ field, onChangeArtistName, onChangeSongTitle }) => {
	...
	const handleSpotifySearch = (query: string) => {
		...
		if (query.length > 2) {
			...
			const filtered = MOCK_SPOTIFY_RESULTS.filter(
				(track) =>
					track.name.toLowerCase().includes(query.toLowerCase()) || track.artists[0].name.toLowerCase().includes(query.toLowerCase())
			);
	...
	const getTrackCardContent = (trackId: string) => {
		...
		return (
			<div className="flex items-center gap-3">
				<Image width={500} height={500} src={track.album.images[0].url} alt={track.album.name} className="w-12 h-12 rounded object-cover" />
				<div>
					<h3 className="font-medium">{track.name}</h3>
					<p className="text-sm text-muted-foreground">by {track.artists[0].name}</p>
